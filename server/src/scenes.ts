...
function cleanAssets(value: SceneAssetRef[] | undefined): SceneAssetRef[] {
  ...
  return (value ?? []).flatMap((asset) => {
    ...
    seen.add(`${kind}:${refId}`);
    return [{ kind, refId, role: text(asset.role) }];
  });
}
...
export async function createSceneTheme(
  ...
  const theme: SceneTheme = { id: randomUUID(), name, description: text(input.description), createdAt: now, updatedAt: now };
...
export async function updateScene(
  ...
  const updated: Scene = {
    ...
    ...(patch.assets !== undefined ? { assets: cleanAssets(patch.assets) } : {}),
