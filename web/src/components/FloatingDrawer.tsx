...
export function FloatingDrawer({ panel, order, children }: { panel: FloatingPanel; order: number; children: ReactNode }) {
  const root = useRef<HTMLDivElement>(null);
  const lastFocus = useRef<HTMLElement | null>(null);
  const wasDocked = useRef(panel.docked);
  const [viewport, setViewport] = useState(() => ({ width: window.innerWidth, height: window.innerHeight }));
  const workbench = useWritingView((s) => s.workbench);
  const gutter = recallGutter(workbench, viewport.width);
  ...
  return <div
    ...
    tabIndex={-1}
    style={{ left, top, width, height, zIndex: 1000 + order, display: panel.docked ? 'none' : 'flex' }}
    ...
    onKeyDown={(event) => {
      ...
      useStore.getState().dockFloatingPanel(panel.id);
    }}
  >
    <header className="floating-drawer-grip">
      ...
      <button type="button" className="floating-drawer-dock" aria-label="Send to side" title="Send to side (Escape)"
        onClick={() => useStore.getState().dockFloatingPanel(panel.id)}>−</button>
    </header>
